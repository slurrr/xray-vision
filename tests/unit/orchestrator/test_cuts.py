...
class TestCutSelector(unittest.TestCase):
    ...
    def test_min_consumed_ingest_seq_tracks_minimum(self) -> None:
        buffer = RawInputBuffer(max_records=10)
        buffer.append(_event("AAA"), ingest_ts_ms=1)
        buffer.append(_event("BBB"), ingest_ts_ms=2)
        buffer.append(_event("BBB"), ingest_ts_ms=3)

        selector = CutSelector()

        self.assertIsNone(selector.min_consumed_ingest_seq())

        selector.next_cut(buffer=buffer, symbol="AAA", cut_end_ingest_seq=1, cut_kind="timer")
        selector.next_cut(buffer=buffer, symbol="BBB", cut_end_ingest_seq=3, cut_kind="timer")

        self.assertEqual(selector.min_consumed_ingest_seq(), 1)
