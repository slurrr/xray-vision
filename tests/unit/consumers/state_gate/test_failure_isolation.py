...
def _config(*, max_gap_ms: int = 1000, publish_max_pending: int = 2) -> StateGateConfig:
    ...
    publish_limits = OperationLimits(max_pending=publish_max_pending, max_block_ms=100, max_failures=3)
...
class TestFailureIsolation(unittest.TestCase):
    ...
    def test_publish_backpressure_halts_and_no_gate_outputs(self) -> None:
        ...
        events = processor.consume(_completed_event("run-3", symbol="TEST", engine_timestamp_ms=105))
        ...
        self.assertEqual(processor.consume(_completed_event("run-4", symbol="TEST", engine_timestamp_ms=200)), [])
