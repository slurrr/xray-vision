...
class TestExecutionPlan(unittest.TestCase):
    def test_deterministic_plan_ordering(self) -> None:
        ...
        signal_b = _Signal(
            "signal.b",
            dependencies=[ModuleDependency(module_id="signal.a", artifact_name="value")],
        )
        rule_r = _Rule(
            "rule.r",
            dependencies=[ModuleDependency(module_id="signal.b", artifact_name="value")],
        )
        output_o = _Output(
            "output.o",
            dependencies=[ModuleDependency(module_id="rule.r", artifact_name="value")],
        )

        registry = ModuleRegistry([output_o, rule_r, signal_b, signal_a])
        plan = build_execution_plan(
            registry,
            enabled_module_ids=["signal.a", "signal.b", "rule.r", "output.o"],
        )
        self.assertEqual(
    ...
    def test_stage_dependency_violation_fails(self) -> None:
        ...
        bad_rule = _Rule(
            "rule.bad",
            dependencies=[ModuleDependency(module_id="output.o", artifact_name="value")],
        )

        registry = ModuleRegistry([signal_a, rule_r, output_o, bad_rule])
        with self.assertRaises(ValueError):
            build_execution_plan(
                registry, enabled_module_ids=["signal.a", "rule.r", "output.o", "rule.bad"]
            )
    ...
    def test_artifact_ordering_is_lexicographic(self) -> None:
        schemas = [
            ArtifactSchema(
                artifact_name="b",
                artifact_schema="schema.b",
                artifact_schema_version="1",
            ),
            ArtifactSchema(
                artifact_name="a",
                artifact_schema="schema.a",
                artifact_schema_version="1",
            ),
        ]
